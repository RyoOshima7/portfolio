...
【何を分析・集計したか】
学生ごとに所属部活数と平均点を集計し、平均点の高い順に並べた。
LEFT JOINを使い、部活に所属していない学生も集計対象に含めた。

【結果から何が言えるか】
学生ごとの部活動への参加状況と成績を一覧で比較できる。
ただし、この集計結果だけでは、部活数と平均点に関係があるとは
断定できないため、追加の分析が必要である。

【次に確認したいデータ】
部活への参加頻度、所属期間、試験科目、試験実施日などを確認したい。
これらを組み合わせて、部活動への参加状況と成績の関係を詳しく分析したい。
*/