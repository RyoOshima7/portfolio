...
## リンク集（Proofの入口）
- **SQL Drills（最新）**：[`sql/drills/2026-03/`](sql/drills/2026-03/)
...
## 結論（1行）
- **SQL drills を q021 まで積み上げ、SELECT / WHERE / GROUP BY / JOIN に加えて、サブクエリ・相関サブクエリ・CASE を使った条件付き集計まで成果物化できた。**

## 次のアクション（1行）
- **次は EXISTS・CASE・サブクエリの使い分けを言語化しつつ、READMEに「1行の解釈」を増やして、就活で説明しやすい形に整える。**
...
| SQL Drills | 2026-03 drills | q013〜q021（LEFT JOIN / 3テーブルJOIN / サブクエリ / EXISTS / 相関サブクエリ / CASE集計） | [sql/drills/2026-03/](sql/drills/2026-03/) |
...
- **今週やったこと**：SQL drills を q019〜q021 まで追加し、HAVING・相関サブクエリ・CASEを使った条件付き集計まで拡張した
- **学び**：単純な抽出だけでなく、**「集計後に絞る」「自分の行に応じて比較先を変える」「条件ごとに件数を分ける」** といった実務で頻出の考え方が見えてきた
- **次週やること**：EXISTS / CASE / サブクエリの違いを1行で説明できるよう整理し、各SQLに「どんな場面で使うか」の観点を足す
