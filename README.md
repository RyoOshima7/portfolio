...
- **Python Basics（最新）**：[`python/basics/q001_sales_total/`](python/basics/q001_sales_total/)
- **ロードマップ原本**：[`roadmap/`](roadmap/)
- **学習メモ（Notes）**：[`notes/`](notes/)
- **分析（準備中）**：[`analysis/`](analysis/)

## 結論（1行）
- **SQL drills に加えて、Pythonでは q001_sales_total を追加し、CSV読み込み・関数・for文・例外処理を使った基礎課題も成果物化できた。**

## 次のアクション（1行）
- **次は Python基礎課題を q002 以降も追加しつつ、SQLと同じように「何を学んだか」をREADMEで説明できる形に整える。**
