...
## 結論（1行）
- **SQL drills を q022 まで積み上げ、SELECT / WHERE / GROUP BY / JOIN に加えて、サブクエリ・相関サブクエリ・CASE を使った条件付き集計とランク付けまで成果物化できた。**

## 次のアクション（1行）
- **次は CASE・HAVING・相関サブクエリの使い分けを言語化しつつ、各SQLに「どんな場面で使うか」の1行説明を足して、就活で説明しやすい形に整える。**
